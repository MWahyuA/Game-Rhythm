...
public class NoteObjectFancy : MonoBehaviour
{
    ...
    void Update()
    {
        if (Input.GetKeyDown(keyToPress))
        {
            ...
            if (canBePressed)
            {
                ...
                {
                    Debug.Log("Hit");
                    GameManagerFancy.instance.NormalHit();
                ...
                {
                    Debug.Log("Good");
                    GameManagerFancy.instance.GoodHit();
                ...
                {
                    Debug.Log("Great");
                    GameManagerFancy.instance.GreatHit();
    ...
    private void OnTriggerExit2D(Collider2D other)
    {
        ...
        {
            canBePressed = false;
            GameManagerFancy.instance.NoteMissed();
            Instantiate(badEffect, transform.position, badEffect.transform.rotation);
            Miss.Play();
            HealthManagerFancy.instance.Damage(1);
            cekMiss = true;
        }
